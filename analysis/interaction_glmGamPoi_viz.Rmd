...
deg <- read_csv("analysis_outs/deg_wilcoxon.csv")[-1]
```
...
## Overview of interaction effects

First, lets take a broad view of the interaction effects
...
p1 <- plot_lfc_pair(c("Rorc"),"IL6_TNF", deg)
p2 <- plot_lfc_pair(c("Thrb","Snn","Irf4","Mdk"),"IL4_TNF", deg)
p3 <- plot_lfc_pair(c("Irf4"),"IL2_TNF", deg)
p6 <- plot_lfc_pair(c("Cxcl10", "Tbx21"),"IFNA_TNF", deg)
p7 <- plot_lfc_pair(c("Cxcl10","Tbx21"),"IL27_TNF", deg)
...
p2 <- ggplot(degSummarySI, aes(x=group,y=interact_freq,fill=num_interactions)) +
  geom_bar(stat = "identity", color = "black") +
  scale_fill_viridis_d(option = "turbo") +
  xlab("ligand") +
...
degSingles <- deg %>%
  filter(grepl("_linker",group)) %>%
  mutate(group = gsub("_linker","",group)) %>%
  filter(pvals_adj < 0.01 & abs(logfoldchanges) > 0.25)

# create tibble with number of significant interaction effects per gene per ligand
groupInteractSummary <- interactions %>%
  filter(adj_pval < 0.01 & abs(lfc) > 0.25) %>%
  separate(interaction, into = c("interaction1", "interaction2"), sep = "_") %>%
  pivot_longer(cols = c(interaction1, interaction2), 
               names_to = "interaction_type", 
               values_to = "group") %>%
...
make_xy_plot("IL4_TNF", interactions = interactionsLfc, singles = singlesLfc, p_val_cutoff=10^-20) +
